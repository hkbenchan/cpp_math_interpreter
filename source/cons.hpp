...
/**
 * \brief Count the number of operand.
 * \param operand The root cell that store the operand
 * \param minimum_op The minimum number of operands that a operation supported
 * \param maximum_op The maximum number of operands that a operation supported
 * \param min_req Indicator of whether there is a minimum
 * requirement of number of the operands
 * \param max_req Indicator of whether there is a maximum
 * requirement of number of the operands
 * \return The number of operand
 */

inline int no_of_operands(Cell* operand,int minimum_op, int maximum_op, 
                          bool min_req, bool max_req)
{
  Cell* operand_ptr = operand;
  int no_of_operand = 0;

  while (!nullp(operand_ptr))
  {
    if (!listp(operand_ptr))
    {
       if (operand_ptr != NULL)
         delete operand_ptr;
       throw runtime_error("Wrong tree structure");
    }
    operand_ptr = cdr(operand_ptr);
    no_of_operand++;

    if (no_of_operand>maximum_op)
      break;
  }

  if ((no_of_operand<minimum_op) && (min_req))
  {
    if (operand_ptr != NULL)
      delete operand_ptr;
    throw runtime_error("Too few argument(s).");
  }

  if ((no_of_operand>maximum_op) && (max_req))
  {
    if (operand_ptr != NULL)
      delete operand_ptr;
  ...
  return no_of_operand;
}


/**
 * \brief Deicde the symbol operator, and then do the corresponding operation.
 * \param op The cell that store the symbol operator
 * \param cdr The root cell that store the operand
 * \return The evaluated result
 */

inline Cell* eval_op(Cell* op, Cell* cdr)
{
  if (symbolp(op))
  {
    SymbolOperator* Symbol_op = Symbol_operator_decider(get_symbol(op));
    return Symbol_op->eval_op(cdr);
  }
  else
  {
    throw runtime_error("No symbol operator exist.");
  }
}
